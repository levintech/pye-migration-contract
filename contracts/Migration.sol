...
contract PYEMigration is Ownable {
    ...
    uint256 public oldTokenRate;
    uint256 public newTokenRate;

    uint256 public oldTokenDecimal;
    ...
    // Update tokens
    function updateTokens(address _oldtoken, address _newtoken, uint256 _oldTokenDecimal, uint256 _newTokenDecimal) external onlyOwner {
        oldtoken = IBEP20(_oldtoken);
        newtoken = IBEP20(_newtoken);

        oldTokenDecimal = _oldTokenDecimal;
        newTokenDecimal = _newTokenDecimal;
    }

    function updateTokensRate(uint256 _oldtokenRate, uint256 _newtokenRate) external onlyOwner {
    ...
    function migration() external {
        ...
        uint256 newtokenAmount = oldtokenAmount.mul(newTokenRate).div(oldTokenRate).mul(newTokenDecimal).div(oldTokenDecimal);
        require(newtoken.balanceOf(address(this)) >= newtokenAmount, "PYEMigration::migration: Insufficient Amount please contact community.");
    ...
    function isWhiteListed(address _account) public view {
